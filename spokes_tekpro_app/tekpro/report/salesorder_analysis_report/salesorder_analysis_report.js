...
frappe.query_reports["SalesOrder Analysis-Report"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
	...
	"formatter": function (value, row, column, data, default_formatter) {
		...
		if (column.fieldname == "delay" && data && data[column.fieldname] > 0) {
		 ...
		 if (column.fieldname == "customer_name" && data &&data["credit_lock"] === "Lock") {
			value = "<span style='color:red;'>" + value + "</span>";
		}
		if(column.fieldname == "customer" && data &&data["credit_lock"] === "Lock") {
			value = "<span style='color:red;'>" + value + "</span>";
		}
