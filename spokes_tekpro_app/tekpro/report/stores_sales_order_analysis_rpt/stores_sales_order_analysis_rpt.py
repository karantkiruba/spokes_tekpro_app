...
def get_conditions(filters):
	...
	if filters.get("customer_group"):
		conditions += " and cus.customer_group = %(customer_group)s"
...
def get_data(conditions, filters):
	data = frappe.db.sql(
		...
		SELECT
			...
			so.status, so.customer, so.customer_name,cus.customer_group, soi.item_code,so.dispatch_instructions,
			...
			inner join `tabPurchase Order Item` pod on pod.parent = po.name
			where pod.item_code = soi.item_code and pod.warehouse = soi.warehouse and po.docstatus=1 and po.status <> 'Cancelled' and po.status <> 'Closed') +
			(SELECT b.actual_qty FROM `tabBin` b where b.item_code = soi.item_code and soi.warehouse = b.warehouse) < (soi.qty - IFNULL(SUM(sii.qty), 0)) THEN 'NOT ORDERED' End) as "ordering_status",
			(CASE WHEN (soi.qty - IFNULL(SUM(sii.qty), 0)) = (SELECT b.actual_qty FROM `tabBin` b where b.item_code = soi.item_code and soi.warehouse = b.warehouse) THEN 'BILL'
		...
		WHERE
			...
			and so.docstatus = 1
			and cus.customer_group in ('Commercial')
			and cus.disabled = 0
			{conditions}
		GROUP BY soi.name
...
def get_columns(filters):
	...
	columns.extend(
		[
			...
			{
				"label":_("Dispatch Instructions"),
				"fieldname":"dispatch_instructions",
				"fieldtype": "Link",
