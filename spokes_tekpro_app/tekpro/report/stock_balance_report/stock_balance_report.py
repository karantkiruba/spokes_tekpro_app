...
def get_stock_ledger_entries(filters: StockBalanceFilter, items: List[str]) -> List[SLEntry]:
	...
	wd  = frappe.qb.DocType("Warehouse Details")

	query = (
		frappe.qb.from_(sle)
		# .left_join(wh)
		# .on((sle.item_code == wh.parent) & (sle.warehouse==wh.warehouse))
		# .left_join(wh)
		# .on(i.name == wh.parent)
		.select(
			sle.item_code,
			sle.warehouse,
			(frappe.qb.from_(wd).select(wd.type).where((wd.parent == sle.item_code) & (wd.warehouse == sle.warehouse)).limit(1)).as_("type"),
			# wh.rack,
			# wh.bin,
			#(select pi.posting_date from `tabPurchase Invoice Item` pii inner join `tabPurchase Invoice` pi on pi.name = pii.parent WHERE pii.item_code = sle.item_code ORDER BY pi.creation DESC LIMIT 1) as last_purchase_date,
			sle.posting_date,
			sle.actual_qty,
			sle.valuation_rate,
			sle.company,
	...
	# wh = frappe.qb.DocType("Warehouse Details")
	# query = (
	# 		query.left_join(wh)
	# 		.on((wh.parent == query.item_code) & (wh.warehouse == sle.warehouse))
	# 		.select(wh.rack,wh.bin)
	# 	)
...
def get_item_details(items: List[str], sle: List[SLEntry], filters: StockBalanceFilter):
	...
	for d in sle:
		...
		.left_join(wh)
		.on((item_table.name == wh.parent) & (d.warehouse == wh.warehouse))
		# .left_join(sle)
		# .on((item_table.name == sle.item_code) & (sle.warehouse == wh.warehouse))
		.select(
			item_table.name,
