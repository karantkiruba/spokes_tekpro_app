...
class ReceivablePayableReport:
	...
	def get_invoices(self, ple):
		if ple.voucher_type in ("Sales Invoice", "Purchase Invoice"):
			if self.filters.get("sales_person"):
				if ple.party in self.sales_person_records.get("Customer", []):
	...
	def set_invoice_details(self, row):
		...
		if row.voucher_type == "Sales Invoice":
			...
			if self.filters.show_sales_person:
				sales_person = frappe.db.get_value("Customer", row.party, "sales_person")
				row.sales_person = sales_person or "Not Assigned"
		...
		if self.account_type == "Receivable":
			...
			if self.filters.show_sales_person:
				...
				sales_team = frappe.db.sql(
					"""
					SELECT SI.name AS parent,C.sales_person
	...
	def apply_customer_user_permissions(self):
		user_perms = frappe.get_user_permissions()
		customers = user_perms.get("Customer")
		if customers:
	...
	def add_common_filters(self):
		...
		if (self.account_type == "Receivable" and not frappe.has_permission("Customer", "read", ignore_share_permissions=True)):
			self.apply_customer_user_permissions()
