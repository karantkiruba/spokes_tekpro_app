...
def _execute(filters=None):
    ...
    if not company:
        frappe.throw(_("Please select Company to generate the report"))
    columns = get_columns(filters)

    output_gst_accounts = [
        account
        for account in get_gst_accounts_by_type(company, "Output").values()
        if account
    ]
    ...
    if item_list:
        itemised_tax, tax_columns = get_tax_accounts(
            item_list, columns, company_currency, output_gst_accounts,filters
        )
    ...
    for d in item_list:
        if (d.parent, d.gst_hsn_code, d.item_code) not in added_item:
            invoice_type = "B2C" if d.gst_category == "Unregistered" else "B2B"
...
def get_items(filters):
    ...
    items = frappe.db.sql(
        ...
        SELECT
            ...
            `tabGST HSN Code`.description,`tabSales Invoice`.gst_category
        FROM
            `tabSales Invoice`
...
def get_merged_data(columns, data):
    ...
    for row in data:
        key = row[0] + "-" + row[2] + "-" + str(row[4]) + "-" + row[-1]
