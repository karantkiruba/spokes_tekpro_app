...
def get_data(filters):
	...
	DateDiff = query_builder.CustomFunction('DATEDIFF', ['schedule_date', 'transaction_date'])
	query = (
		frappe.qb.from_(po)
		.from_(po_item)
		.left_join(pi_item)
		...
		.left_join(bin)
		.on((bin.item_code == po_item.item_code)&(bin.warehouse == po_item.warehouse))
		.select(
			po.transaction_date.as_("date"),
			po_item.schedule_date.as_("required_date"),
			DateDiff(po_item.schedule_date,functions.CurDate()).as_("delay_in_days"),
			po_item.project,
			po.name.as_("purchase_order"),
			po.status,
			po.supplier,
			po_item.item_code,
			po_item.description.as_("description"),
			po_item.qty,
			po_item.received_qty,
			bin.actual_qty.as_("current_stock"),
	...
	columns.extend(
		[
			...
			},
			{
				...
				{"label": _("Delay In Days"), "fieldname": "delay_in_days", "fieldtype": "Data", "width": 90},
			{
				"label": _("Warehouse"),
				"fieldname": "warehouse",
				"fieldtype": "Link",
