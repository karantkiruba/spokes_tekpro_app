...
def execute(filters: Optional[StockBalanceFilter] = None):
	...
	item_type_detail_map = get_item_type(item_map.keys())
	...
	for group_by_key in iwb_map:
		...
		if item_map.get(item):
			...
			if item + warehouse in item_type_detail_map:
				item_type = item_type_detail_map[item + warehouse]["type"]
...
def get_item_warehouse_map(filters: StockBalanceFilter, sle: List[SLEntry]):
	...
	#item_type = frappe.qb.DocType("Warehouse Details")
	#query = (
			#qry.left_join(item_type)
			#.on((item_type.parent == sle.item_code) & (item_type.warehouse == sle.warehouse))
...
def get_item_type(items):
	item_type_details = frappe._dict()

	if items:
		item_type_details = frappe.get_all(
			"Warehouse Details",
			["parent", "warehouse", "type"],
			filters={"parent": ("in", items)},
		)

	return dict((str(d.parent) + str(d.warehouse), d) for d in item_type_details)
